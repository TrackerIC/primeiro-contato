...
colunas_com_padrao = [
    coluna for coluna in planilha.columns if pd.Series(coluna).str.match(padrao).any()
]
...
# GRAFICO
if (entrada_Dia >= diaMin and entrada_Dia <= diaMax) and (
    entrada_TCU > 0 and entrada_TCU <= numero_de_colunas_com_padrao
):
    # Pega o valor do TCU
    listaPosTCU = arquivo_selec[
        f"ncu.tcus[{entrada_TCU}].data.Position_a1_rad_s1"
    ].tolist()
    ...
    messagebox.showinfo(
        "Alerta", f"O número do dia é menor que {diaMin} ou maior que {diaMax}"
    )
