...
def convert(arquivo):
    ...
    for i in range(0, 12):
        dataframe[f"ncu.tcus[{i}].data.Position_a1_rad_s1"] = np.degrees(
            dataframe[f"ncu.tcus[{i}].data.Position_a1_rad_s1"]
        )
    ...
    print(os.getcwd())
    return os.path.join(os.getcwd(), novo_nome_arquivo)
